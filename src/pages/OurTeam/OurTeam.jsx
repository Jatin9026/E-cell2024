...
// export default OurTeam;import { motion } from "framer-motion";
import { motion } from "framer-motion";
...
function OurTeam() {
  ...
  // Helper to get display role (prefer position, fallback to domain)
  const getDisplayRole = (member) => {
    if (member.position && member.position.trim() !== "") return member.position;
    if (member.domain && member.domain.trim() !== "") return member.domain;
    return "Team Member";
  };
  const getYearOrder = (year) => {
    if (!year) return 99;
    const yearStr = year.toString().toLowerCase();
    if (yearStr.includes("3rd") || yearStr === "3") return 1; // 3rd year first
    if (yearStr.includes("2nd") || yearStr === "2") return 2; // 2nd year second
    if (yearStr.includes("1st") || yearStr === "1") return 3; // 1st year third
    if (yearStr.includes("4th") || yearStr === "4") return 4; // 4th year last
    return 99;
  };
  const sortByYear = (membersArray) => {
    return [...membersArray].sort((a, b) => getYearOrder(a.year) - getYearOrder(b.year));
  };

  // Grouping functions
  const isOverallCoordinator = (m) =>
    m.position?.toLowerCase() === "overall coordinator";
  const isDeputyCoordinator = (m) =>
    m.position?.toLowerCase().includes("deputy coordinator");
  const isSecretariat = (m) =>
    ["general secretary", "finance secretary"].includes(m.position?.toLowerCase());
  const isDomainManager = (m) =>
    m.position?.toLowerCase().includes("manager") && !isDeputyCoordinator(m);
  const isTechnical = (m) =>
    m.domain?.toLowerCase() === "technical" &&
    !isOverallCoordinator(m) &&
    !isDeputyCoordinator(m) &&
    !isSecretariat(m) &&
    !isDomainManager(m);
  const isPR = (m) =>
    m.domain?.toLowerCase() === "public relations" || m.domain?.toLowerCase() === "pr";
  const isGraphics = (m) => m.domain?.toLowerCase() === "graphics";
  const isCorporate = (m) => m.domain?.toLowerCase() === "corporate";
  const isEvents = (m) => m.domain?.toLowerCase() === "events";

  // Apply sorting after filtering
  const overallCoordinators = sortByYear(members.filter(isOverallCoordinator));
  const deputyCoordinators = sortByYear(members.filter(isDeputyCoordinator));
  const secretariats = sortByYear(members.filter(isSecretariat));
  const domainManagers = sortByYear(members.filter(isDomainManager));
  const technicalMembers = sortByYear(members.filter(isTechnical));
  const prMembers = sortByYear(members.filter(isPR));
  const graphicsMembers = sortByYear(members.filter(isGraphics));
  const corporateMembers = sortByYear(members.filter(isCorporate));
  const eventsMembers = sortByYear(members.filter(isEvents));

  // Render a single member card
  const MemberCard = ({ member }) => {
    const hasImage = member.profile_pic && member.profile_pic.trim() !== "";
    const linkedInUrl = member.linkedin_url;
    const email = member.email || member.kiet_email;
        ...
        <div className="relative">
          {hasImage ? (
            <img
              ...
              alt={member.name}
              className="team-member-img w-48 h-60 rounded-tr-[10%] rounded-lg rounded-bl-[10%] object-cover"
              loading="lazy"
            />
          ) : (
            <div className="team-member-img w-48 h-60 rounded-tr-[10%] rounded-lg rounded-bl-[10%] bg-gray-200 dark:bg-[#000000] flex items-center justify-center text-center p-4">
              <span className="text-black dark:text-white font-semibold text-lg break-words">
                {member.name}
              </span>
            </div>
          )}
          <div className="teamHover absolute top-5 left-5 w-[70%] border-t-4 border-l-4 p-3 pt-8 pb-5 border-blue-500 rounded-md flex flex-row justify-evenly items-center">
            {linkedInUrl && (
              <a href={linkedInUrl} target="_blank" rel="noreferrer">
                <img src={linkedInIcon} alt="LinkedIn" className="w-9 h-9" loading="lazy" />
              </a>
            )}
            {email && (
              <a href={`mailto:${email}`} target="_blank" rel="noreferrer">
                <img src={gamilIcon} alt="Email" className="w-8 h-8" loading="lazy" />
              </a>
            )}
          </div>
        </div>
  ...
  const Section = ({ title, members }) => {
    ...
    return (
      <>
        <div className="mt-10">
          <h1 className="text-xl md:text-3xl font-bold text-black dark:text-white text-center px-4">
            {title}
          </h1>
        </div>
        <div className="flex flex-wrap justify-center items-start mt-8">
  ...
  const Loader = () => (
    <div className="flex justify-center items-center min-h-screen bg-white dark:bg-black">
      <div className="relative w-16 h-16">
        ...
        <div className="absolute top-0 left-0 w-full h-full border-4 border-t-[#4d55ba] rounded-full animate-spin"></div>
      </div>
    </div>
  );
  ...
  if (error) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-white dark:bg-black">
        <div className="text-red-500 text-xl">Failed to load team: {error}</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col justify-center items-center text-white pt-28 bg-white dark:bg-black">
      ...
      <Section title="Overall Coordinators" members={overallCoordinators} />
      <Section title="Deputy Coordinator" members={deputyCoordinators} />
      <Section title="Administrative & Financial Secretariat" members={secretariats} />
      <Section title="Domain Managers" members={domainManagers} />
      <Section title="Technical Member" members={technicalMembers} />
      <Section title="Public Relations Member" members={prMembers} />
      <Section title="Graphics Member" members={graphicsMembers} />
      <Section title="Corporate Member" members={corporateMembers} />
      <Section title="Event Member" members={eventsMembers} />
    </div>
  );
}
